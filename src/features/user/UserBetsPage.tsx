    ...
    const loadUserBets = async (id: number) => {
        try {
            ...
            const filteredBets = bets
                .filter(b => typeof b.userId === 'number' && b.userId === id)
                .map(b => ({
                    ...b,
                    ...
                    match: {
                        ...b.match,
                        matchTime: b.match.matchTime ? new Date(b.match.matchTime) : new Date()
                    }
                })) as UserBetWithMatch[];
            setUserBets(filteredBets);
            if (filteredBets.length > 0) {
                setUserName(filteredBets[0].userName || '');
                setUserImageUrl(filteredBets[0].userImageUrl || '');
            }
            setError(null);
        } catch (err) {
